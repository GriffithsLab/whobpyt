...
class ModelFitting(AbstractFitting):
    """
    ...
    def train(self, u, empRecs: list, 
        ...
        for i_epoch in range(num_epochs):
            ...
            if self.model.use_fit_gains:
                if self.model.model_name == 'JR':
                    self.trainingStats.appendSC_p2p(self.model.sc_p2p.detach().cpu().numpy())
                    self.trainingStats.appendSC_p2e(self.model.sc_p2e.detach().cpu().numpy())
                    self.trainingStats.appendSC_p2i(self.model.sc_p2i.detach().cpu().numpy())
                else:
