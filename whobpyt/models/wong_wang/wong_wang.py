...
from whobpyt.datatypes import AbstractNeuralModel, AbstractParams, Parameter
...
class ReducedWongWangParams(AbstractParams):
    ...
    def __init__(self, **kwargs):
        ...
        param = {
            ...
            "std_in": Parameter(0.02),  # standard deviation of the Gaussian noise
            "std_out": Parameter(0.02),  # standard deviation of the Gaussian noise
            # Parameters for the ODEs
            # Excitatory population
            "W_E": Parameter(1.),  # scale of the external input
            "tau_E": Parameter(100.),  # decay time
            "gamma_E": Parameter(0.641 / 1000.),  # other dynamic parameter (?)

            # Inhibitory population
            "W_I": Parameter(0.7),  # scale of the external input
            "tau_I": Parameter(10.),  # decay time
            "gamma_I": Parameter(1. / 1000.),  # other dynamic parameter (?)

            # External input
            "I_0": Parameter(0.32),  # external input
            "I_external": Parameter(0.),  # external stimulation

            # Coupling parameters
            "g": Parameter(20.),  # global coupling (from all nodes E_j to single node E_i)
            "g_EE": Parameter(.1),  # local self excitatory feedback (from E_i to E_i)
            "g_IE": Parameter(.1),  # local inhibitory coupling (from I_i to E_i)
            "g_EI": Parameter(0.1),  # local excitatory coupling (from E_i to I_i)
            ...
            "k1": Parameter(2.38),
            "k2": Parameter(2.0),
            "k3": Parameter(0.48),  # adjust this number from 0.48 for BOLD fluctruate around zero
            "V": Parameter(.02),
            "E0": Parameter(0.34),
            "tau_s": Parameter(1 / 0.65),
            "tau_f": Parameter(1 / 0.41),
            "tau_0": Parameter(0.98),
            "mu": Parameter(0.5)
