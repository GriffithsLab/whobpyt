...
def model_arg_type_check(model_init):
    """
    Takes the self.__init__ object of a given model class (e.g. RNNJANSEN) and checks that the passed arguments abide by their 
    expected data types.  If there is a discrepency, raises a ValueError.
    """

    args_signature = inspect.signature(model_init)
