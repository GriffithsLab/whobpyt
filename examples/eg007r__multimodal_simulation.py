...
from whobpyt.models.RWWNEURO.ParamsRWWNEUinSec import ParamsRWWNEU
...
eeg_data = np.zeros((eeg.shape[0]-3, eeg.shape[1], eeg.shape[2]+1))
eeg_data[:,:,:-1] = eeg[3:,:,]*1e5
...
eeg_mean =dataloader(np.concatenate(list(eeg_data), 1).T, num_epochs, 21*eeg_data.shape[2])
...
params_rww = ParamsRWWNEU(g=par(400, 400, 1/np.sqrt(10), True), g_EE=par(1.5, 1.5, 1/np.sqrt(50), True), \
                  ...
                  g_IE=par(np.log(0.6), np.log(0.6), 0.1, True, True), I_0 =par(0.2), \
                   std_in=par(np.log(0.05), np.log(0.05), 0.1, True, True), std_out=par(0.00))
...
# ---------------------------------------------------
#
F.train(u = 0, empRec = bold_mean, empRecSec=eeg_mean, num_epochs = num_epochs, TPperWindow = TPperWindow, warmupWindow=0, learningrate = 0.05)

F.evaluate(u = 0, empRec = bold_mean, empRecSec=eeg_mean, TPperWindow = TPperWindow, base_window_num = 1)
