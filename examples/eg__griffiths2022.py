...
===================================================================
DL based parameter estimation for neurophysiological models
===================================================================

This example contains the replication results of the paper: Griffiths, J. D., Wang, Z., Ather, S. H., Momi, D., Rich, S., Diaconescu, A., McIntosh, A. R., & Shen, K. Deep Learning-Based Parameter Estimation for Neurophysiological Models of Neuroimaging Data. bioRxiv (2022). DOI: 10.1101/2022.05.19.492664
...
from whobpyt.depr.griffiths2022.plotting import (
    get_rotation_matrix,
...
json_path = os.path.join(eg_dir, 'regime_search_I0_result.json')

#  Load the JSON content
with open(json_path, 'r') as f:
    data = json.load(f)

# 2.1 Now access I0 and c_I0
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# 
I0 = data['I0']
c_I0 = data['c_I0']
...
# 2.2 fixed I_0 = 0.2 coastly search g_EE
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# 
...
# %%
# **Result Description:**
# The bifurcation analysis results show the dynamics of the RWW model that change as different parameters are tested. 
# The dynamics of the single node network are shown using the eigenvalues that can be used to represent stability.
...
eg_dir = fetch_eggriffiths2022()
sc_file = os.path.join(eg_dir, 'hagmann', 'weights.txt') #CS
...
base_dir = os.path.join(eg_dir, 'hcp')  # CS
...
base_dir = os.path.join(eg_dir, 'hcp')       
out_dir = os.path.join(eg_dir, 'hagmann')    
for i in range(0,40):
...
for i in range(39):
    ...
    Syn_ts_test[sub] = np.loadtxt(os.path.join(hagmann_dir, f'bold_test_{sub}.txt'))
    Syn_ts_sim[sub] = np.loadtxt(os.path.join(hagmann_dir, f'bold_train_{sub}.txt'))
...
# Save results
np.save(os.path.join(hagmann_dir, 'Syn_ts_sim.npy'), Syn_ts_sim)
np.save(os.path.join(hagmann_dir, 'Syn_ts_test.npy'), Syn_ts_test)
np.save(os.path.join(hagmann_dir, 'Syn_sc_mod.npy'), Syn_sc_mod)
np.save(os.path.join(hagmann_dir, 'Syn_sc.npy'), Syn_sc)
np.save(os.path.join(hagmann_dir, 'Syn_fitparas.npy'), Syn_paras)

# %%
# 3.3 Plot Post Distributions of the fitted model parameters
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
mask = np.tril_indices(66,-1)
node_size = 83
...
eg_dir = fetch_eggriffiths2022()
sc_file = os.path.join(eg_dir, 'hcp', 'weights.txt')
sc = np.loadtxt(sc_file)
sc = (sc + sc.T)*0.5
SC = sc - np.diag(np.diag(sc))
...
eg_dir = fetch_eggriffiths2022()
base_dir = os.path.join(eg_dir, 'hcp')
ts_file = os.path.join(eg_dir, 'SYN_bold_test_output.txt')  # make sure this file exists
ts = np.loadtxt(ts_file)
print(ts.shape)

# Set the structural connectivity file
sc_file = os.path.join(base_dir, 'weights.txt')
...
eg_dir = fetch_eggriffiths2022()
base_dir = os.path.join(eg_dir, 'hagmann')
...
# 4.1: Analysis of the HCP data model-fitting
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
...
# Set base_dir to the 'hcp/Outputs' subfolder
base_dir = os.path.join(eg_dir, 'hcp', 'Outputs')

# Define file paths
fc_file = os.path.join(base_dir, 'HCP_ts.npy')                  # functional connectivity
sc_file = os.path.join(base_dir, 'HCP_sc.npy')                  # structural connectivity
fc_sim_file = os.path.join(base_dir, 'HCP_ts_sim.npy')          # simulated FC
fc_sim_idt_file = os.path.join(base_dir, 'HCP_ts_sim_idt.npy')  # simulated FC (identified params)
sc_mod_file = os.path.join(base_dir, 'HCP_sc_mod.npy')          # modified SC
sc_mod_idt_file = os.path.join(base_dir, 'HCP_sc_mod_idt.npy')  # modified SC (identified)
paras_file = os.path.join(base_dir, 'HCP_fitparas.npy')         # parameters
paras_idt_file = os.path.join(base_dir, 'HCP_fitparas_idt.npy') # parameters (identified)
fc_test_file = os.path.join(base_dir, 'HCP_ts_test.npy')        # FC test
fc_test_idt_file = os.path.join(base_dir, 'HCP_ts_test_idt.npy')# FC test (identified)
...
# %%
# 4.2 Plot the connectivity weight matrices
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
...
# %%
# 4.3 Model parameter fitting estimation
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
start_time = time.time()
...
# %%
# 5. Generate the plots of the default mode network structure.
# ---------------------------------------------------
#
...
# References
# ---------------------------------------------------
#Deep Learning-Based Parameter Estimation for Neurophysiological Models of Neuroimaging Data. (2022). In Health & Medicine Week (pp. 1830-). NewsRX LLC.
